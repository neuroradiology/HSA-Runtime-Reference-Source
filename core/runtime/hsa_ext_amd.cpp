...
hsa_status_t HSA_API hsa_ext_code_unit_load(
    hsa_runtime_caller_t caller, const hsa_agent_t* agent, size_t agent_count,
    void* serialized_code_unit, size_t serialized_code_unit_size,
    const char* options, hsa_ext_symbol_value_callback_t symbol_value,
  ...
  hsa_status_t hsa_error_code = core::HsaCodeUnit::Create(
      &hsa_code_unit, caller, agent, agent_count, serialized_code_unit,
      serialized_code_unit_size, options, symbol_value);
...
hsa_status_t HSA_API hsa_ext_code_unit_destroy(hsa_amd_code_unit_t code_unit) {
  auto code_unit_pointer = std::find(std::begin(loaded_code_units),
                                     std::end(loaded_code_units), code_unit);
...
hsa_status_t HSA_API
    hsa_ext_code_unit_get_info(hsa_amd_code_unit_t code_unit,
...
hsa_status_t HSA_API hsa_ext_code_unit_iterator(
  ...
  for (size_t i = 0; i < num_loaded_code_units_at_the_moment; ++i) {
    hsa_status_t hsa_error_code =
        code_unit_iterator(caller, loaded_code_units[i]);
    if (HSA_EXT_STATUS_INFO_HALT_ITERATION ==
        (hsa_amd_status_t)hsa_error_code) {
      return HSA_STATUS_SUCCESS;
    }
    if (HSA_STATUS_SUCCESS != hsa_error_code) {
      return hsa_error_code;
